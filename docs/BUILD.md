...
`tests/` má jeden soubor na rodinu (`ctr`, `sec`, `mut`, `idm`, `wf`, `res`, `int`, `evd`, `arch`, `zlab`, `zlab-durable`, `zlab-mirror`, `zlab-import` pro Žlab nad `node:sqlite`, `facts` a `plan` pro slovník faktů a deterministický planner), `mail` pro druhý tok, `inst` pro fail-closed načtení instalačního profilu a workflow definic (Test ID řezu `INST-001..005`, ne normy), `fakes` pro `document.validate` přes protokol `apf-fakes` (Test ID řezu `INT-HTTP-001..008`: stejné třídy chyb jako `INT-FAIL` přes skutečnou síť, chaos přepínač bez rebuildu, nedosažitelný dvojník i nesmyslná 200 odpověď, protokol DMS a archivu pro celek D) a `reg` pro Agent Registry (Test ID řezu `REG-001..004`: `Router.catalog()` se neliší od `providers()`, nese risk/isolation metadata z descriptoru, `capabilityNamesOf()` je jediný zdroj pro cross-Worker dispatch tabulky). Názvy testů nesou Test ID z `VERIFICATION-CONTRACT.md`. Conformance fixtures a golden výstupy jsou v `conformance/<capability>/`, golden mastery workflow v `conformance/workflows/<workflow>/`. Testy staví svět přes `tests/harness/index.ts` (`createSlice(volby)` = instalace `config/local-fakes` + fake secrets). Testy zapisující journal a audit soubory používají dočasný adresář systému; nic nezůstává v repu.
