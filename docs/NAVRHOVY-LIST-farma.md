...
8. **Vstupní formáty faktur a normalizace** (pokyn vlastníka 6. 9. 2026 večer: „faktury jsou nejvíce v PDF a jako příloha e-mailu", ISDOC XML, „i takovéto formáty"). Importní vrstva pozná formát **deterministicky** a normalizuje do jednoho interního modelu faktury podle **EN 16931** (sémantický model, na kterém stojí Peppol BIS 3.0 i Factur-X); AI extrakce zůstává jen pro obyčejné PDF a fotky. Pořadí pro ČR: **ISDOC / ISDOCX** (isdoc.cz; `DocumentType` 1–6 = rodina faktur; ISDOCX = zip s `.isdoc` + přílohy) → **Factur-X / ZUGFeRD** (PDF/A-3 s vloženým XML: vyjmout přílohu z PDF dřív, než se sáhne po modelu) → **UBL 2.1 / Peppol BIS Billing 3.0** → **UN/CEFACT CII** → **EDIFACT INVOIC** (jen bude-li zákazník) → CSV/JSON/proprietární XML jako konektory. Capability `invoice.normalize` (deterministická, `sideEffects: none`, výstup = Invoice JSON s `source` a provenancí; pole: číslo, datumy, strany vč. IČ/DIČ, řádky, DPH, celkem, platební údaje vč. IBAN/VS); pro PDF bez XML navazuje `invoice.extract` nad textem z `toMarkdown` (AI, `usesLlm`, F2 hranice, druhý signál = kontrola součtů a formátů). Detekce formátu podle namespace/kořenového elementu, ne podle přípony (`faktura.xml` může být UBL, CII i proprietární). Souvisí s projektem faxx-dox (extraktor dokumentů), kde se totéž jádro použije.
   **Potvrzeno vlastníkem 6. 9. 2026 večer („přesně takhle bych to navrhl") jako „Universal Invoice Intake Gateway":** tok `e-mail → intake → detekce příloh → {ISDOC/ISDOCX, UBL, CII → přímý parser | Factur-X → vyjmout XML z PDF/A-3 | PDF jen vizuální, JPG/PNG → OCR/AI | GRID/EDI payload → EDI adaptér (OpenText Trading Grid: EDI, SFTP/AS2, Peppol, portál, e-mail)} → kanonický Invoice JSON → validace → kontrola duplicity → business rules → ERP (NAV / Business Central)`. **Pravidlo přednosti:** na fakturu se strojovými daty se AI nepouští; přijde-li `faktura.pdf` + `faktura.isdoc` v jednom mailu, zpracuje se ISDOC (XSD validace proti schématu DIA, aktuálně 6.0.2), PDF se uloží jen jako vizuální originál; `confidence = deterministic`. Vazby na normu: kontrola duplicity = idempotence podle identity faktury (DIČ dodavatele + číslo dokladu) jako `idempotencyKey` write executora; business rules = deterministický `validate` s druhým signálem (součty, formáty IČ/DIČ/IBAN, ARES); zápis do NAV/BC = write executor `erp.post` (PRINCIPAL, reconciliace přes BC API) = **třetí doména** pro měření mezní ceny (posudky). Adaptéry: EmailAdapter, GRIDAdapter, PeppolAdapter, ISDOCParser, UBLParser, CIIParser, FacturXParser, PdfAiExtractor, ImageAiExtractor; za nimi jediný interní kontrakt (`invoice.v1` JSON: documentType, invoiceNumber, supplier{companyId, vatId, name}, customer, issueDate, dueDate, currency, payment{variableSymbol, iban}, totals{net, vat, gross}, lines[]).

   **Rozpracování pole a ověření proti státním registrům (pokyn vlastníka 7. 9. 2026, ještě nezapočato — plán, ne kód):**

   **a) Pole `invoice.v1` s provenancí, ne jen ploché hodnoty.** Skupiny polí: identifikace dokladu (číslo faktury, variabilní symbol, číslo objednávky, číslo smlouvy) · dodavatel (název, IČO, DIČ, adresa, IBAN) · odběratel (název, IČO, DIČ, adresa) · datumy (vystavení, DUZP, splatnost) · částky (základ daně, DPH podle sazeb, celkem bez/s DPH, zaokrouhlení, k úhradě) · měna · platební údaje (účet, IBAN, SWIFT/BIC, VS/KS/SS) · položky (název, množství, jednotka, cena/jednotka, sleva, sazba DPH, cena bez/s DPH) · reference (objednávka, dodací list, zakázka, středisko) · další (způsob úhrady, doprava, poznámka, kontakt). Každé pole neseno jako `{value, confidence, sourcePage, sourceBoundingBox, normalizedValue, validationStatus}`, ne holá hodnota — tohle je přesně vzor `FieldValue<T>` (`value/source/confidence/trustLevel`), který `document-classifier`/`document-validator` používají už dnes (viz `src/components/*/handler.ts`), jen rozšířený o `sourcePage`/`sourceBoundingBox` (kde na stránce se to našlo) a `normalizedValue` (např. datum na ISO, částka na číslo). Minimální sada pro ERP import: dodavatel, IČO/DIČ, číslo faktury, datum vystavení, DUZP, splatnost, měna, základ DPH, DPH, celkem, variabilní symbol, objednávka.

   **b) Ověření tuzemského subjektu proti autoritativním registrům — SAMOSTATNÁ capability, ne součást extrakce.** Vlastníkovo zdůvodnění (přesně tak, jak padlo): „tohle bych určitě nemíchal do OCR/extraction agenta — je to samostatná capability, deterministický kontrolní worker, ne volná úvaha AI." Tok: `invoice.extract`/`invoice.normalize` vytáhne IČO/DIČ/účet → nová capability ověří: IČO a základní údaje v ARES, stav plátce DPH, zda subjekt není veden jako nespolehlivý plátce, zda je bankovní účet na faktuře zveřejněný u Finanční správy (vlastník: „Finanční správa pro to má přímo webovou službu určenou SW třetích stran, vrací mimo jiné spolehlivost plátce a zveřejněné bankovní účty" — **needs verification**: přesný název/endpoint/kontrakt téhle služby jsem si k dnešnímu dni neověřil, ověřit v `docs/SHODA-NIS2-ISO27001.md` duchu než se začne psát kód, ne převzít bez ověření). **Rozhodnuto 7. 9. 2026 (vlastník): rozdělit.** `cz.company.verify` (ARES, IČO, právní forma, adresa) a `cz.vat.verify` (DPH plátcovství, nespolehlivý plátce, zveřejněný účet) jako dvě samostatné kapability, ne jedna `cz.subject.verify`. Důvody: jiný externí systém pro každou (ARES vs. Finanční správa, jiné chaos/dostupnostní módy); `cz.company.verify` se hodí u jakéhokoli dokumentu se firmou, ne jen u faktur; subjekt nemusí být plátce DPH — pak `cz.vat.verify` vrací "nevztahuje se", ne selhání, a to je jiná sémantika než binární ARES existuje/neexistuje. Výsledek jako druhý deterministický signál vedle dnešního `document.validate` (stejná rodina jako W4), ne jako blokující hradlo bez možnosti review. Kam v pořadí: až po kroku 8 (invoice.normalize/extract musí existovat dřív, tohle nad jejich výstupem), ale před `erp.post` (třetí doména) — logicky **krok 8b**, dosud nezařazený do číslovaného pořadí výše.

   **Univerzální vs. agendové kapability (vlastníkův postřeh 7. 9. 2026, potvrzeno):** `document.classify`/`validate`/`stamp` běží u každého dokumentu bez ohledu na typ — univerzální. `cz.company.verify`/`cz.vat.verify` mají naopak běžet jen pro **nakonfigurovaný seznam agend** (návrh: faktura, objednávka, smlouva — ne účtenka ze šatny nebo osobní poznámka). Ten seznam agend má být **konfigurovatelný** (`config/<instalace>/`, stejná zásada jako instalační profil), ne napevno zadrátovaný do jedné workflow definice — jinak by přidání nové agendy (např. „i dodací listy") vyžadovalo zásah do kódu místo do configu. Objednávka dnes nemá vlastní `documentType` (spadá do `OTHER`) — pokud se agendový seznam má opřít o `documentType`, chce to nejdřív rozšířit enum (`INVOICE`/`CONTRACT`/`ORDER`/`OTHER`), samostatné rozhodnutí, zapsat zvlášť až padne.
