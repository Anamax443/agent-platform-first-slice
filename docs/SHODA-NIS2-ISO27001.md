...
| Kryptografie a klíče (A.8.24; NIS2 21(2)(h)) | Obálky podepsané Ed25519 (JCS kanonizace), privátní klíč jen jako Worker secret, veřejný klíč v overlay instalace, okna platnosti a grace period v `KeyRegistry` (SEC-CRED-002/003); TLS a šifrování at rest zajišťuje Cloudflare; **od celku D2 (6. 9. 2026) hosty veřejný klíč skutečně dostávají** a ověřují proti němu obálku na skutečném Workeru, ne jen v testu | `src/platform/signing.ts`, `config/farm-bass443/farm.json` (`$signingPublicKeys`), HANDOFF (17), (25) | rotace klíče ještě neproběhla v provozu (postup: nový `keyId` + nový secret + redeploy hostů) |
| Logování a audit (A.8.15, A.5.28; NIS2 21(2)(b),(f)) | Audit append-only: v objektu instance (SQLite) a kopie ve společné D1; každý záznam má aktéra, čas UTC, druh, korelaci; mazání instance zanechá záznam `PURGED` s aktérem a důvodem; `EVD-004` hlídá, že audit nemá update/delete API. **Nalezeno a opraveno 7. 9. 2026 (W23):** relay auditních záznamů z hostu na sdílenou D1 (`ctx.waitUntil` bez čekání volajícího) uměl potichu ztratit záznamy o zápisu — write proběhl, doklad o něm v append-only auditu ne, beze stopy chyby na obou stranách. Opraveno (`RelayAudit.flush()` — odpověď hostu se vrátí až po doručení nebo viditelném selhání relay), ověřeno živě i regresním testem `DH-AUDIT-RELAY-001` | `deploy/cloudflare/apf-gateway/src/store.ts`, `deploy/cloudflare/apf-document-host/src/relay-audit.ts`, `/audit.json`, testy `EVD-*`, `DH-AUDIT-RELAY-001`, MEASUREMENT W23 | export/retence auditu (90 dní podle profilu) není automatizovaná; alert kanál při `STORAGE_FULL` a bezpečnostních událostech chybí (W10); centrální log mimo Cloudflare (SIEM) není |
