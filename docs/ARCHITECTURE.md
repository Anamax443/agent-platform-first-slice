...
Na farmě je **jeden Durable Object per instance toku** (`deploy/cloudflare/apf-gateway/src/`): jeho SQLite je journal, audit i úložiště artefaktů té instance, synchronně, takže orchestrátor běží uvnitř objektu a každý přechod je trvalý před dalším voláním (RES-CRASH-001). R2 (originály pod `originals/<tenant>/<sha256>`) a D1 (společný audit, `/audit.json`) dostávají asynchronní kopie až po běhu; objekt je zdroj pravdy (W15). Platformová rozhraní `JournalStore`, `AuditTrail` a `ArtifactWriter` zůstávají synchronní záměrně; host v jiném Workeru si artefakt z R2 přednačte před voláním handleru. Dokud není router zapojený, `NotWiredTransport` vrací každému kroku `DEPENDENCY_UNAVAILABLE`, aby instance skončila explicitně a auditovaně, nikdy tichým úspěchem. Stránky `/` a `/workflow/<id>` jsou server-rendered HTML bez skriptů: evidence, ne aplikace. Binární originál (PDF, fotka, docx) jde nejdřív neměnně do R2 pod `originals/<tenant>/<sha256>` a objekt drží jen jeho metadata (`location`); Workers AI (`toMarkdown`) z něj vytěží text, který je uložen jako derivace s provenancí (`derivedFrom`, `producer`, audit `document.extract`), a tok běží nad ní. Capability tedy nikdy nedostane binární data; extrakce zatím není capability s descriptorem (W16). Strukturované e-faktury (ISDOC, později UBL/Peppol, CII, Factur-X) jdou deterministickou cestou bez modelu (návrhový list, krok 8).

**Celek D2: vzdálený executor host jako skutečný Worker.** `apf-document-host` (`document.stamp`, `document.archive`) přestal být skeleton a běží jako samostatný deployable, dosažitelný z gatewaye jen přes `RemoteHostTransport` s podepsanou obálkou (D1, výše). Protože host nemá lokální úložiště, artefakt si asynchronně natáhne z gatewaye (`GET /workflow/:id/artifact/:id`, service binding) **před** spuštěním synchronního `Router`/`ExecutorHost` řetězce — hranice mezi „platforma dodá data" a „executor je synchronně zpracuje" je tak explicitní, ne skrytá uvnitř handleru (`SingleArtifactStore` v `deploy/cloudflare/apf-document-host/src/index.ts`). Veřejný podpisový klíč gatewaye dostane host jako `SIGNING_PUBLIC_KEYS` var, naplněnou z `farm.json`'s `$signingPublicKeys` generátorem instalace.

**Spolehlivost auditu napříč Workery (W23).** Host relayuje svůj lokální audit (dispatch, write-intent, write-done) na sdílený D1 přes `POST /audit` na gatewayi. Fire-and-forget `ctx.waitUntil()` samo o sobě nezaručovalo doručení: isolát uměl být recyklován dřív, než slib doběhl, beze stopy chyby na obou stranách — write samotný proběhl, ale doklad o něm v append-only auditu chyběl. `RelayAudit` (`deploy/cloudflare/apf-document-host/src/relay-audit.ts`, vyňatá z `index.ts` kvůli testovatelnosti bez wrangler-only aliasu) proto eviduje každý rozjetý relay a `/dispatch` handler čeká na `flush()` těsně před odpovědí, v obou větvích (úspěch i chyba) — odpověď se nikdy nevrátí, dokud audit doopravdy nedorazí nebo viditelně neselže. `append()` sám zůstává synchronní (`AuditTrail` kontrakt, FOUNDATION-core §7, beze změny). Obecné poučení pro normu: cross-Worker fire-and-forget bez explicitního bodu, kde na něj volající počká, není bezpečný vzor pro nic append-only.

**Operátorský pohled: `/farm`.** Nad sdíleným D1 auditem a nad `stub.view()` jednotlivých Durable Objectů (ne nad auditní relay, která je jen kopie) stojí stránka `apf-gateway/src/page.ts`'s `renderFarm()`: zdraví všech pěti Workerů (`/version` přes service bindingy, `MAIL_INGEST` binding přidán jen pro tenhle účel — router ho dál nedispatchuje), detail kroků posledních instancí přímo z DO a čitelný „deník" (poslední řádky D1 `audit` tabulky, stejný zdroj jako `/audit.json`). Vizuál je banka z vlastníkova katalogu `Anamax443/Interface-Par` (`deploy/cloudflare/apf-gateway/src/bank.ts`, styl `saas-modern`, rozvržení `side-nav`, doslovná kopie `bank/ui.css` + `bank/tokens/style/saas-modern.css`) — samostatný vizuální jazyk od zbytku gatewaye, který zůstává bez skriptů jako evidence. Vendorovaná písma vynechána (systémový fallback stačí pro jednoho operátora); malý vanilla JS jen přepíná viditelnost panelů podle `location.hash` a sbaluje boční menu do `data-layout="rail"`.

**Modely AI jsou věc instalace (W17):** profil má sekci `models` per AI capability (provider `workers-ai` | `anthropic` | `fake`, id modelu, credential jen jménem, `inferenceGeo`, `processor`); `modelTable()` rozliší dostupné a nedostupné volby, nedostupný výchozí model zastaví wiring („nikdy bez modelu"). Gateway staví adaptéry (`src/adapters/workers-ai.ts`, `src/adapters/anthropic.ts`) z profilu, strategie `llm` = výchozí model, `keyword` = pravidla jako záložní strategie a druhý signál; formulář nabízí volbu per dokument, workflow v2 ji nese jako `$input.model`, provenance výsledku nese `modelId` a `promptVersion`. Podpis obálek: Ed25519 klíč ze secretu (`node:crypto` ve workerd ověřeno), veřejný klíč v overlay instalace; instalace bez `apiHost` smí běžet na efemérním klíči.
