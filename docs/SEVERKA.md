...
| **Admission Gate (module lifecycle)** | **první krok hotový (9. 9. 2026)** — `LifecycleRegistry` (`src/platform/lifecycle.ts`), `Router` ho vynucuje před `checkGrant()` (`MODULE_QUARANTINED`), volitelný `config/<installation>/lifecycle.json`, wire-nuto live na `apf-document-host` (`docs/POSUDKY.md` Posudek 7, rozhovor o "koupi nových krav" 9. 9. 2026) | ruční block-list (`ACTIVE`/`QUARANTINED` per modul), ne mandatorní allow-list — chybějící záznam = `ACTIVE`, nemění chování dnešních běžících komponent | nízké dnes (ruční, jeden host). Zbývá: `NEW`/`TESTING`/`DEGRADED` stavy, automatický verification runner nad `verificationProfiles` z `module-descriptor.v1.schema.json` (ten už dnes deklaruje `riskClass`→`isolationClass` pravidla a `buildCommit`, jen je nic nevynucuje běhu), auto-quarantine na živém selhání, rozšíření na zbylé hosty (email-executor, mail-ingest, gateway in-process) |
