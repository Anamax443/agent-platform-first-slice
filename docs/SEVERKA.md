...
| **Human Review** | rozhodovací cesta existuje a je živě ověřená | — | **Opraveno 8. 9. 2026 (`6dea224`, HANDOFF 50–52), tenhle řádek byl zastaralý — popisoval stav před opravou jako aktuální.** `SqliteReviewTaskStore` (Durable Object SQLite) drží úkoly durabilně, `orchestratorFor()` teď sdílí jednu instanci `ReviewService` napříč požadavky, `POST /workflow/:id/review/decide` → `decideReview()` je nasazený a živě ověřený na `farm-bass443` (celý cyklus `WAITING(REVIEW)` → decide → dokončená instance, ne uvíznutí). **Zbývá:** `orchestrator.applyReviewExpiries()` (WF-REV-003 — `EXPIRE_TO_FAILED`/`EXPIRE_TO_CANCELLED`/`ESCALATE`/`CREATE_NEW_REVIEW`) existuje a je otestovaný (`tests/wf.test.ts`), ale nic na farmě ho nevolá — `apf-gateway`'s `scheduled()` řeší jen R2 inbox. Ruční rozhodnutí funguje, časové expirace na farmě dnes nikdy samy nenaskočí |
