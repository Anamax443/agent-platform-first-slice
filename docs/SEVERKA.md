...
- **Žlab** — formální jméno pro to, co je výš i níž popsané jako „sklad": **immutable, signed**
  ...
  `src/platform/evidence.ts`'s `EvidenceLedger`, `tests/zlab.test.ts` (HANDOFF 121). **Živě zapojeno a
  durable od 15. 9. 2026** (M0 část D, HANDOFF 162–167): SQLite objektu instance + insert-only zrcadlo v D1,
  záznam v2 s podpisem `EVIDENCE:v2:`, `cz.company.verify`/`cz.vat.verify` na farmě reálně zapisují,
  `/farm/zlab.json?verify=1` ověří kopii v D1 jen veřejným klíčem. **Trusted `EvidenceWriter` hotov 13. 9. 2026** (Posudek 15
...
| **M3** | **No-n8n Gate** | compile `plan → WorkflowDef`, `available` počítané ze Žlabu (hash + TTL + doména), živě. **Killer test bez BC:** (1) Žlab bez platné evidence `cz.company.verify` → plán obsahuje verify; (2) čerstvá evidence správné domény → verify z plánu zmizí; (3) prošlá / cizí tenant / špatná doména → verify se vrátí. Bez editace jediného workflow. Adversarial: podvržené `available` klíče. **Compiler mechanismus hotový a testovaný 19.9.2026** (część 6 krok 8, `src/platform/compiler.ts` + `compiler-policy.ts`, viz `AUTONOMOUS-RUNTIME-V1.md`) — killer test (1)/(2) doložen na úrovni compileru přes reálný katalog (`cz.company.verify` se objeví/zmizí podle `document.type.invoiceConfirmed`); živé zapojení do gateway a skutečná `HttpAresAdapter`/`HttpMojeDaneAdapter` volání (M4/M5, sloupec vpravo) zůstávají TARGET | 5, 6 (skutečná `HttpAresAdapter`/`HttpMojeDaneAdapter` volání) |
...
2. **Evidence/Provenance Contract — Žlab** (Posudek 12 bod 11, viz `### Kontrola musí být svázaná
   ...
   `apf.maxferit.cz`. **Od 15. 9. 2026 i s `EvidenceWriter` živě** (M0 část D, HANDOFF 162–167): Žlab je
   durable (SQLite objektu instance + insert-only zrcadlo v D1, záznam v2 s podpisem `EVIDENCE:v2:`), obě
   krávy na farmě reálně zapisují, `buildHash` = `GIT_SHA` nasazení (R5), `/farm/zlab.json?verify=1` ověří
   kopii v D1 jen veřejným klíčem — dřívější výhrada „evikce může tiše ztratit" je tím uzavřená. V testovacím
   řezu `src/slice.ts` `buildHash` zůstává jen
