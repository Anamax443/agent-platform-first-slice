...
|---|---|---|---|
| **Agent Registry** | **první krok hotov (9. 9. 2026):** `src/platform/registry.ts` (`capabilityNamesOf()`, `catalogEntry()`) + `Router.catalog()` — `platform-wiring.ts`'s dřív ručně duplikovaná pole (`GATEWAY_CAPABILITIES`/`DOCUMENT_HOST_CAPABILITIES`/`EMAIL_EXECUTOR_CAPABILITIES`) teď odvozená přímo z `descriptor.json` každého modulu, ne psaná znovu vedle sebe (`tests/reg.test.ts`, REG-001..004). Chybí: HTTP `/registry`/`/capabilities` endpoint (deklarovaný v descriptorech, dosud nikde neimplementovaný), nahrazení natvrdo psaných `capabilities:[...]` v `/version` handlerech na farmě, health/cena | formalizace toho, co už komponenty nesou v `descriptor.json` (capabilities, vstup/výstup schema) + health/verze/cena | nízké — je to datová nadstavba nad existujícím vzorem |
