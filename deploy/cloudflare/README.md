...
**Historický stav (6. 9. 2026 večer; aktuální stav je vždy v `HANDOFF.md`, tenhle odstavec je ponechán jako záznam): nasazeno na bass443; krok 1 hotový, krok 3 „lite" hotový, krok 2 celky A, A2, B a C hotové.** Celek B: gateway podepisuje obálky Ed25519 klíčem ze secretu `GATEWAY_SIGNING_KEY` (veřejný klíč `k1` v `config/farm-bass443/farm.json`), router v objektu instance obsluhuje `document.classify` (modely ze sekce `models` profilu: výchozí Workers AI Llama 3.1 8B fp8, volitelně Llama 3.3 70B, Anthropic Opus 5 / Haiku 4.5 po nastavení `ANTHROPIC_API_KEY`; výběr per dokument ve formuláři, nedostupné volby jmenují chybějící secret) a `document.validate` (registr = `apf-fakes` přes skutečný service binding, protokol a chaos módy v `src/adapters/fakes-http.ts`; DMS a archiv v tomtéž dvojníku čekají na document-host, celek D); `stamp` a dál končí `DEPENDENCY_UNAVAILABLE` (hosty = celek D). Náhled chaos přepínačů: `GET /chaos` na gateway i přímo na `apf-fakes`; nastavuje se `wrangler kv key put registry.mode <mode> --namespace-id <CHAOS>` (klíče `registry.mode`, `registry.delayMs`, `dms.mode`, `dms.status`), bez redeploye. Modely přidáš tak, že rozšíříš `models` v profilu a nastavíš secret: `npx wrangler secret put ANTHROPIC_API_KEY -c .wrangler/generated/farm-bass443/apf-gateway/wrangler.jsonc`, pak `farm-deploy`. Pět Workerů běží z generovaných configů instalace `farm-bass443`; gateway je na `apf.maxferit.cz` za Cloudflare Access a má **příjem dokumentu**: stránka `/` (soubor PDF / fotka / docx / txt / eml do 4 MB, nebo vložený text; tok; text razítka) → `POST /intake`. Textový originál jde do SQLite Durable Objectu instance (kopie do R2 pod `originals/<tenant>/<sha256>`); **binární originál jde nejdřív do R2** (neměnně, klíč = sha256) a Workers AI (`AI.toMarkdown`) z něj vytěží text, který se uloží jako **derivace s provenancí** (`derivedFrom` = originál, `producer` = `workers-ai:toMarkdown`, audit `write-intent`/`write-done` pro `document.extract`); tok běží nad derivací. Selhání extrakce = stránka 422 s otiskem originálu, tok se nespustí. Instance toku žije v témže objektu (journal, audit i artefakty synchronně, RES-CRASH-001), kopie auditu do D1 (`/audit.json`), stránka instance `/workflow/<id>` (+ `.json`). Router a podpis obálky ještě nejsou zapojené: každý krok toku končí po vyčerpání technických retry explicitně `DEPENDENCY_UNAVAILABLE` a instance je FAILED. `/version` a `/health` hlásí `wired` po částech. Hosty nemají routu, jsou dostupné jen přes service bindingy. `apf-gateway` má navázanou instalaci (`apf:installation`, sestavená fail-closed při importu); totéž běží na `wrangler dev` pro `local-fakes`.
...
| `apf-fakes` | DMS, registr, archiv jako dvojníci za skutečnou sítí, chaos přepínače; **zapojeno do `document.validate` (celek C)** | testovací dvojník | KV `apf-chaos`, KV `apf-fakes-store` | `DMS_SECRET`, `ARCHIVE_SECRET` (očekávané hodnoty; registr je bez credentialu) |

Co je skutečné a co simulace, po napojení:

| Skutečné na farmě | Stále simulace |
|---|---|
| hranice PRINCIPAL (samostatný Worker, vlastní bindingy) | data DMS a registru (odpovědi dvojníka, ne skutečný systém) |
| podepsaná obálka přes service binding / HTTP | at-least-once doručení, dokud není fronta (Workers Paid) |
| `document.validate` → `apf-fakes` přes service binding, chaos v KV bez redeploye | zápisy DMS a archivu (čekají na document-host, celek D) |
