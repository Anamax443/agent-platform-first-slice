// Durable Object backing ExecutorHost's idempotency store on this Worker (SEVERKA.md "Audit provenance" /
// Execution Engine row, docs/POSUDKY.md Posudek 7 MAJOR 4). apf-email-executor is otherwise stateless:
// ExecutorHost is rebuilt fresh on every /dispatch and its default in-memory IdempotencyStore dedups nothing
// across requests or isolates. One object per dedup key (idFromName in index.ts) gives the atomic
// check-then-reserve the design needs for free, because Cloudflare serializes every request to the same
// Durable Object instance — a genuine concurrent duplicate delivery cannot race past the reservation the way
// a stateless D1 read-then-write could.
// Duplicated from apf-document-host/src/idempotency-ledger.ts rather than shared across deployables — each
// deployable stays self-contained, same convention as the rest of this repo's deploy/cloudflare tree
// (relay-audit.ts follows the identical pattern).
