    ...
    capabilities: [
      {
        ...
        handler: companyVerify.createCompanyVerifier({
          ...
          // TENANT_WIDE (docs/AUTONOMOUS-RUNTIME-V1.md część 2): a company's ARES status doesn't depend on which
          // Case asked — bound on the writer's identity, never on a per-call claim.
          evidence: new EvidenceWriter(evidence, { producerId: "cz.company.verify", capabilityVersion: "1", buildHash, reusePolicy: "TENANT_WIDE", ...authorityOf("cz.company.verify") }, clock),
        }),
      },
    ],
  });
    ...
    capabilities: [
      {
        ...
        handler: vatVerify.createVatVerifier({
          ...
          // TENANT_WIDE for the same reason as cz.company.verify above — DIČ reliability doesn't depend on the Case.
          evidence: new EvidenceWriter(evidence, { producerId: "cz.vat.verify", capabilityVersion: "1", buildHash, reusePolicy: "TENANT_WIDE", ...authorityOf("cz.vat.verify") }, clock),
        }),
      },
    ],
  });
